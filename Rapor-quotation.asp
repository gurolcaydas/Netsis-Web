    ...
    <div class="container-fluid" style="margin-top:80px"> 
        <!-- #include file="./subs/dbcon.asp" -->
                <form class="form-horizontal" method="POST" action="?doo=list">
                    <div class="container-fluid p-4"> <h3>Quotation</h3><h5>Sales Price</h3>
        ...
        <button class="btn btn-success m-2" onclick="html_table_to_excel('xlsx','filename','tblData')"><i class="bi bi-file-earmark-arrow-down-fill"></i> Excel</button>
            <table class="table table-sm table-striped table-hover align-middle"  id="tblData">         <%
                ' SQL   
                    ...
                    Netsis_SQL=Netsis_SQL+"  Y.STOK_KODU as 'SKU #' "
                    Netsis_SQL=Netsis_SQL+"  ,Y.STOK_ADI as 'Description TR' "
                    Netsis_SQL=Netsis_SQL+" ,A2.[INGISIM] as 'Description EN' "
                    Netsis_SQL=Netsis_SQL+"  ,Z.[FIYAT1] as 'Price'  "
                    Netsis_SQL=Netsis_SQL+"  ,Z.[FIYATDOVIZTIPI] AS 'Currency'    "
                    ...
                    Netsis_SQL=Netsis_SQL +" LEFT JOIN ["+currentDB+"].[dbo].[TBLSTSABITEK] A2  with (NOLOCK) ON Y.[STOK_KODU]=A2.[STOK_KODU] "
