    ...
    <div class="container-fluid" style="margin-top:80px"> <%
        ...
        if url_doo="" or url_doo="list" then
                ...
                <form class="form-horizontal" method="POST" action="?doo=list">
                    <div class="container-fluid p-4"><h4>Fiyat listesi satırları</h4>         
                        <div class="input-group">
                            <div class="col-1"><input class="form-control" type="text" name="search_AS"  placeholder="A/S Alış/Satış"  value="<%=search_AS%>"></div>
                            ...
                            <textarea class="form-control z-depth-1" name="search_bisiklet_kodu" rows="1" placeholder="Stok kodu"><%=search_bisiklet_kodu%></textarea>
            ...
            if LEN(search_bisiklet_kodu)>0 then  
                aranacak_stok_kodu=Replace(search_bisiklet_kodu,vbCrLf, " ")        
                aranacak_stok_kodu=Replace(aranacak_stok_kodu, "‚", " ")
                aranacak_stok_kodu=Replace(aranacak_stok_kodu, "‚", " ")
                i=0
                Do While i<>LEN(aranacak_stok_kodu)
                        i=LEN(aranacak_stok_kodu)
                        aranacak_stok_kodu=Replace(aranacak_stok_kodu, "  ", " ")
                Loop
                aranacak_stok_kodu=trim(aranacak_stok_kodu)
                aranacak_stok_kodu="'"+Replace(aranacak_stok_kodu, " ", "','")+"'"
            end if
            'Response.Write (aranacak_stok_kodu&"****")
        ...
        if url_doo="list" and (search_bisiklet_kodu&search_bisiklet<>"" OR request.querystring("detay")=1) then 'alanlar boş ise sakın arama'
            ' SQL 
                ...
                if len(aranacak_stok_kodu)>0 then     
                        Netsis_SQL=Netsis_SQL+" AND [STOK_KODU] IN ("&aranacak_stok_kodu&") " 
                end if 
            ...
            sira=0 
                do until NetsisRecordSet.EOF OR sira>=5000
                    ...
                    <tr>
                        ...
                        %> </td>
                        <td class="small"><%=NetsisRecordSet("BASTAR")%> - <%=NetsisRecordSet("BITTAR")%></td>
                    </tr>                             <%
        ...
        if url_doo="list" and (search_bisiklet_kodu&search_bisiklet="" and search_cari_kod&search_cari&search_liste_kodu&search_AS<>"") then 'Sadece cari ve liste numaması ile arama'
            ' SQL 
                Netsis_SQL= " SELECT TOP 5000 "   
                Netsis_SQL=Netsis_SQL + " A.[FIYATLISTEKODU] "   
                Netsis_SQL=Netsis_SQL + " ,A.[A_S] "   
