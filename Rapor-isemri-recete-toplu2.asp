           ...
           a=Split(str)
            for each xisemri in a
                ...
                <h3>DİKKAT! bu alan yeni:</h3><h4><%=search_bom%> Ürge reçetesi ve <%=herisemri%> İş Emri reçetesi farkları:</h4>
                    ...
                    Netsis_SQL = Netsis_SQL + "              LEFT JOIN ["+currentDB+"].[dbo].TBLISEMRIREC AS T6 ON  T6.ISEMRINO = '" + herisemri + "'  AND T6.HAM_KODU=A.HAM_KODU  "
                    Netsis_SQL = Netsis_SQL + "              LEFT JOIN ["+currentDB+"].[dbo].TBLSTOKURM AS T5 ON T5.HAM_KODU=A.HAM_KODU AND   T5.MAMUL_KODU='"+search_bom+"'   AND T5.STOK_MALIYET ='S' "
