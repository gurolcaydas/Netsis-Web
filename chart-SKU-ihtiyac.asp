      ...
      <div>
        ...
        aranan=request.querystring("SKU")
                ' SQL   
                    ...
                    Netsis_SQL = Netsis_SQL + "	case WHEN A.STHAR_FTIRSIP =6 then 'Customer Order' ELSE 'Supply Order' end  as 'Status' "
                    ...
                    Netsis_SQL = Netsis_SQL + " WHERE    A.STHAR_HTUR='H' AND A.DEPO_KODU!='62' AND A.SUBE_KODU='1'  AND (B.KAPALI='H' OR B.KAPALI IS NULL) AND (case WHEN A.STHAR_FTIRSIP =6 then -1 ELSE +1 end) * (isnull([STHAR_GCMIK],0) - isnull([FIRMA_DOVTUT] ,0))!=0 "

                    Netsis_SQL = Netsis_SQL + "union all "
                    Netsis_SQL = Netsis_SQL + "	SELECT "
                    Netsis_SQL = Netsis_SQL + "		'On Hand (Depo 101 ve 66 hariç)' as 'Status' "
                    ...
                    'Netsis_SQL = Netsis_SQL + "		LEFT JOIN ["+currentDB+"].[dbo].[TBLSTOKPH] ST2 ON ST2.[STOK_KODU]=A.STOK_KODU AND ( ST2.[SUBE_KODU]=1 OR  ST2.[SUBE_KODU]=2) and  ( ST2.[DEPO_KODU]=66  )"
                    'Netsis_SQL = Netsis_SQL + "		LEFT JOIN ["+currentDB+"].[dbo].[TBLSTOKPH] ST3 ON ST3.[STOK_KODU]=A.STOK_KODU AND ( ST3.[SUBE_KODU]=1 OR  ST3.[SUBE_KODU]=2) and  (ST3.[DEPO_KODU]=101  )"
                    ...
                    Netsis_SQL = Netsis_SQL + "Select SKU,Descr,ItemGroup,Whenn,sum(isnull(QTY,0)) as Cumulative, Status from Liste   "
                    Netsis_SQL = Netsis_SQL + "WHERE SKU LIKE '"&aranan&"' "
                    Netsis_SQL = Netsis_SQL + "GROUP BY SKU,Descr,ItemGroup,Whenn, Status "
