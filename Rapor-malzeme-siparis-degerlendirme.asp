    ...
    search_madde_kodu=trim(search_stok_kodu) %>         
        <div class="container-fluid" style="margin-top:80px"> 
            ...
            <div class="container-fluid p-4"> <h3></h3>                
                ...
                <div class="container-fluid">Stok durumu Depo 101 ve 66 hariçtir.</div>
                <%  
                if url_doo="list" and len(search_stok_kodu)>0 then   %>
                    ...
                    <table class="table table-sm table-striped table-hover align-middle" id="tblData">         <%
                        ' SQL   
                            ...
                            Netsis_SQL = Netsis_SQL + " 			,C.[ISEMRINO] as 'Info1'  "
                            Netsis_SQL = Netsis_SQL + " 			,D.TARIH as 'Whenn' "
                            Netsis_SQL = Netsis_SQL + " 			,C.[MAMUL_KODU] as 'Info2' "
                            ...
                            Netsis_SQL = Netsis_SQL + "          case WHEN A.STHAR_FTIRSIP =6 then 'MusteriSip' ELSE 'TedarikSip' end  as 'Status' "
                            ...
                            Netsis_SQL = Netsis_SQL + " 	WHERE  A.STHAR_HTUR='H' AND A.DEPO_KODU!='62' AND A.SUBE_KODU='1' AND (B.KAPALI='H' OR B.KAPALI IS NULL)  AND (isnull(A.[STHAR_GCMIK],0) - isnull(A.[FIRMA_DOVTUT] ,0) )>0 " 
                            Netsis_SQL = Netsis_SQL + " union all " 
                            Netsis_SQL = Netsis_SQL + " 	SELECT "
                            ...
                            Netsis_SQL = Netsis_SQL + " 		,EN.ING as 'Item Group' "
                            Netsis_SQL = Netsis_SQL + " 		,B2.GRUP_ISIM as Kod2  "
                            Netsis_SQL = Netsis_SQL + " 		,CASE WHEN ST.[SUBE_KODU]=2 THEN 'PnA Depo' ELSE 'Depoda' END  as 'Nerede' "
                            Netsis_SQL = Netsis_SQL + " 		,'' as 'Durum' "
                            Netsis_SQL = Netsis_SQL + " 		,D.DEPO_ISMI as 'BoM' "
'                            Netsis_SQL = Netsis_SQL + " 		,CASE WHEN [TOP_GIRIS_MIK] IS NULL THEN 0 ELSE [TOP_GIRIS_MIK] END - CASE WHEN [TOP_CIKIS_MIK] IS NULL THEN 0 ELSE [TOP_CIKIS_MIK] END AS 'Stok_Miktar' "
                            Netsis_SQL = Netsis_SQL + "		,isnull(ST.[TOP_GIRIS_MIK],0) - isnull(ST.[TOP_CIKIS_MIK],0) AS 'Stok_Miktar' "
...
'                            Netsis_SQL = Netsis_SQL + " 		LEFT JOIN ["+currentDB+"].[dbo].[TBLSTOKPH] ST ON ST.[STOK_KODU]=A.STOK_KODU AND ( ST.[SUBE_KODU]=1 OR  ST.[SUBE_KODU]=2) and  ST.[DEPO_KODU]=0  "
                            Netsis_SQL = Netsis_SQL + "		LEFT JOIN ["+currentDB+"].[dbo].[TBLSTOKPH] ST ON ST.[STOK_KODU]=A.STOK_KODU AND ( ST.[SUBE_KODU]=1 )  and  (ST.[DEPO_KODU]!=0 AND ST.[DEPO_KODU]!=101 AND ST.[DEPO_KODU]!=66)   "
                            Netsis_SQL = Netsis_SQL + "		LEFT JOIN  ["+currentDB+"].[dbo].[TBLSTOKDP] D ON D.DEPO_KODU=ST.DEPO_KODU  "
                            ...
                            Netsis_SQL = Netsis_SQL + "			,R.SIPARIS_NO as Source "
                                ...
                                if len(search_madde_kodu)>0 then 
                                    if instr(search_madde_kodu," ") then 
                                        ...
                                        for each x in a2
                                            ...
                                            Netsis_SQL=Netsis_SQL+" SKU LIKE '"+x+"'"
                                            y=1
                                        next
                                        Netsis_SQL=Netsis_SQL+") "
                                    else
                                    Netsis_SQL=Netsis_SQL+" AND SKU LIKE '"&search_madde_kodu&"' "
                                    end if
                                end if 
                            ...
                            ' madde adına göre ara
                                ...
                                if instr(search_madde_ad," ") then 
                                    ...
                                    for each x in a3
                                        if y=1 then Netsis_SQL=Netsis_SQL+" OR "
                                        Netsis_SQL=Netsis_SQL+" Description LIKE '"+x+"'"
                                        y=1
                                    next
                                    Netsis_SQL=Netsis_SQL+") "
                                else
                                Netsis_SQL=Netsis_SQL+" AND Description LIKE '"&search_madde_ad&"' "
                                end if
                                end if 
                        ...
                        NetsisRecordSet.Open Netsis_SQL, NetsisConnection ,0,1 
                            ...
                            do until NetsisRecordSet.EOF 'OR sira>=5000
                                ...
                                next
                                toplam=toplam+cdbl(NetsisRecordSet("QTY"))
                                if toplam<0 then toplamclass="bg-warning" else toplamclass=""
                                Response.Write("<td class='"&toplamclass&"'>" & toplam & "</td>")
