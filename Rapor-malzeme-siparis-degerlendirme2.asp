...
Netsis_SQL = Netsis_SQL + "      union all   "
Netsis_SQL = Netsis_SQL + "      SELECT  "
Netsis_SQL = Netsis_SQL + "          'Stok' as 'stst'  "
...
'Netsis_SQL = Netsis_SQL + "          ,CASE WHEN [TOP_GIRIS_MIK] IS NULL THEN 0 ELSE [TOP_GIRIS_MIK] END - CASE WHEN [TOP_CIKIS_MIK] IS NULL THEN 0 ELSE [TOP_CIKIS_MIK] END AS 'Stok_Miktar'  "
                            Netsis_SQL = Netsis_SQL + "		,isnull(ST.[TOP_GIRIS_MIK],0) - isnull(ST.[TOP_CIKIS_MIK],0)-isnull(ST2.[TOP_GIRIS_MIK],0) + isnull(ST2.[TOP_CIKIS_MIK],0) AS 'Stok_Miktar' "
...
'Netsis_SQL = Netsis_SQL + "          LEFT JOIN ["+currentDB+"].[dbo].[TBLSTOKPH] ST ON ST.[STOK_KODU]=A.STOK_KODU AND ( ST.[SUBE_KODU]=1 OR  ST.[SUBE_KODU]=2) and  ST.[DEPO_KODU]=0   "
                            Netsis_SQL = Netsis_SQL + "		LEFT JOIN ["+currentDB+"].[dbo].[TBLSTOKPH] ST ON ST.[STOK_KODU]=A.STOK_KODU AND ( ST.[SUBE_KODU]=1 OR  ST.[SUBE_KODU]=2) and  ST.[DEPO_KODU]=0  "
                            Netsis_SQL = Netsis_SQL + "		LEFT JOIN ["+currentDB+"].[dbo].[TBLSTOKPH] ST2 ON ST2.[STOK_KODU]=A.STOK_KODU AND ( ST2.[SUBE_KODU]=1 OR  ST2.[SUBE_KODU]=2) and  (ST2.[DEPO_KODU]=101 OR ST2.[DEPO_KODU]=66  )  "
...
Netsis_SQL = Netsis_SQL + "  	PIVOT (sum(QTY) FOR stst IN (Stok,MusteriSip,TedarikSip,Ihtiyac,UretimEmri)) AS P                        "
                        ...
                        NetsisRecordSet.Open Netsis_SQL, NetsisConnection ,0,1 
                            ...
                            do until NetsisRecordSet.EOF 'OR sira>=5000
                                ...
                                next
                                if isnull(NetsisRecordSet("Stok")) then a1=0 else a1=cdbl(NetsisRecordSet("Stok"))
